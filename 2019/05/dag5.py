# -*- coding: utf-8 -*-
"""
Created on Thu Apr  2 13:55:40 2020
...
def get_values(liste, num, op_code, modes):
  mode_a, mode_b, mode_c = modes
  values = []
  
  if op_code in ["01", "02", "04"]:
    if mode_c == "0":
        values.append(liste[liste[num+1]])
    else:
        values.append(liste[num+1])
  
    if op_code in ["01", "02"]:
        if mode_b == "0":
            values.append(liste[liste[num+2]])
        else:
            values.append(liste[num+2])
  
        if op_code in []:
            if mode_a == "0":
                values.append(liste[liste[num+3]])
            else:
                values.append(liste[num+3])
  return values

def perform_calc(input_liste):
...
#  liste[2]= verb
  ...
  while liste[num] != 99:
    op_code, modes = split_instructions(liste[num])
    values = get_values(liste, num, op_code, modes)
    if op_code == "01": #addition
      liste[liste[num+3]]= values[0] + values[1]
      num += 4
      
    elif op_code == "02":#Multiplikation
      liste[liste[num+3]]= values[0] * values[1]
      num += 4
      
    elif op_code == "03":#input
      var_b = pre_def_input#input("intast et tal: ")
      while not is_int(var_b):
        var_b = input("input ikke int. Indtast ny int: ")
      liste[liste[num+1]] = var_b
      num +=2
...
result = perform_calc(puzzle_input)
print(result)
